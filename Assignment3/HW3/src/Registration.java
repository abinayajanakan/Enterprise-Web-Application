...
public class Registration extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String prod_name="lenovo thinkpad series 3";
		float price=670;
		int qty=154;
		int sale=0;
		int rebate=1;
		...
		if(!password.equals(repassword)){
			...
			try
			{
				hm=MySqlDataStoreUtilities.getCustomers();
			}
			catch(Exception e)
			{
			}
			...
			if(hm.containsKey(username))
				...
				MySqlDataStoreUtilities.insertCustomer(username, password,usertype);
				MySqlDataStoreUtilities.insertupdated(prod_name, price,qty,sale,rebate);
				if (usertype.equals("customer")) {
